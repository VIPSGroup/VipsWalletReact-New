...
const {loggedInUser}= useSelector(state=>state.login)
  ...
  const clickSendOtp = (e) => {
    ...
    if (recieverNo.length == 10) {
      if (referName) {
        if (amount) {
          sendMoneyOtp(loggedInUser.Mobile, loggedInUser.TRXNPassword, recieverNo, amount).then(
            (response) => {
  ...
  const clickSendMoney = (e) => {
    e.preventDefault();
    setLoading(true);
    sendMoney(loggedInUser.Mobile, loggedInUser.TRXNPassword, recieverNo, amount, otp).then(
      (response) => {
        if (response.ResponseStatus == 1) {
          setLoading(false);
          setIsSnackBar(true);
