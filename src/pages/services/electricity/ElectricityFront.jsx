...
const ElectricityFront = ({ props }) => {
  ...
  useEffect(() => {
    ...
    getElectricityOperators().then((response) => {
      response && setOperatorList(sortOperator(response.Data));
    });
  }, [props]);

  const getSubdivision = (ourCode) => {
    getSubdivisionData(ourCode).then((response) => {
      setSubdivision(sortSubdivision(response.Data));
      setSelectSubdivision(response.Data[0].Name);
      setSubdivisionCode(response.Data[0].SubDivisionCode);
    });
  };
  ...
  const sortSubdivision = (subdivision) => {
    let _sortSubdivision = subdivision.sort((a, b) =>
      a.Name > b.Name ? 1 : -1
    );
